...
export const findTeam = (root: CEO | Head | Team, id: string): Team | undefined => {
  if (root.id === id) {// @ts-ignore here the id we will pass will always be of the team so root returned will always be a team and hence we can assign it while ignoring the warning
    return {...root};
  }
